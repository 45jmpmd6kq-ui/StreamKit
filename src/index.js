// StreamKit — point d'entree.
//
// Enchainement au demarrage :
//   dossiers -> config -> journal -> modules decouverts -> Twitch -> serveur
...
// s'ouvrir. Un streamer qui a un souci doit TOUJOURS pouvoir arriver sur la page
// et lire le journal pour comprendre -- c'est la que se joue le support.
...
// C'est le seul point de contact entre un module et le reste du monde. Un
// module ne lit jamais un fichier de config lui-meme, n'ouvre jamais de
// connexion Twitch, et ne cree jamais son propre serveur.
...
function contextePour(m) {
  ...
  return {
    ...
    // Minuteurs suivis : ils sont coupes automatiquement quand le module
    // s'arrete. Sans ca, desactiver un module laisserait ses setInterval
    // tourner jusqu'au redemarrage.
    minuteur: {
      intervalle(fn, ms) {
...
// On garde le contexte de chaque module pour pouvoir le nettoyer a l'arret.
...
const app = {
  ...
  async reconnecterTwitch() {
    ...
    for (const id of contextes.keys()) contextes.get(id)?._nettoyer();
    contextes.clear();
  ...
  // Adresse de retour a declarer dans l'app Spotify/Riot du streamer.
  urlDeRetourModule(id) {
    return 'http://localhost:' + PORT + '/callback/module/' + id;
  },

  // --- Reglages generaux ---

  async definirReglagesGeneraux({ depotMaj }) {
    const c = store.getConfig();
    c.maj ??= {};
    if (depotMaj !== undefined) c.maj.depot = String(depotMaj).trim();
    store.sauverConfig(c);
    log.info('Depot de mise a jour : ' + (c.maj.depot || 'aucun'));
    return { ok: true, depotMaj: c.maj.depot };
  },

  // --- Mise a jour ---
  verifierMaj: () => maj.verifier(),
  appliquerMaj: () => maj.appliquer(),
};
